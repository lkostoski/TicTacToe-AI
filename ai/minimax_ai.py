...
def minimax_score_with_cache(board, player):
    board_cache_key = str(board) + "_" + player
    ...
    if board_cache_key not in cache:
        ...
        for move in available_moves:
            new_board = [row[:] for row in board]
            new_board[move[0]][move[1]] = player
            score = minimax_score_with_cache(new_board, opponent)
            scores.append(score)
        if player == 'X':
...
def minimax_ai(board, player):
    ...
    for move in available_moves:
        new_board = [row[:] for row in board]
        new_board[move[0]][move[1]] = player
        score = minimax_score_with_cache(new_board, 'O' if player == 'X' else 'X')
